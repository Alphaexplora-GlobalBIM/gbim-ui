...
export default function Navbar() {
  ...
  const navLinks = [
    {
      ...
      dropdown: [
        {
          name: "Pre-Engineered Building Detailing",
    ...
    },
    {
      name: "About Us",
      path: "/about",
      dropdown: [{ name: "Meet Our Team", path: "/about/our-team" }],
    },
  ...
  return (
    ...
    >
      <div className="w-full px-6 lg:px-12">
        <div className="flex justify-between items-center h-16">
          ...
          <Link to="/" className="flex items-center gap-4 group">
            ...
            <div className="flex flex-col">
              <div className="text-2xl font-bold uppercase tracking-widest text-white leading-none">
                ...
                <span className="text-yellow-500 group-hover:text-yellow-400 transition-colors duration-300 ease-out">
                  Global
                </span>
                BIM
              </div>
          ...
          <div className="hidden md:flex space-x-6 lg:space-x-10 items-center">
            {navLinks.map((item) => (
              <div key={item.name} className="relative group">
                ...
                <Link
                  to={item.path}
                  className={`flex items-center gap-1 text-xs lg:text-sm font-bold transition-colors duration-300 ease-out uppercase tracking-widest ${
                    scrolled
                ...
                >
                  ...
                  {item.dropdown && (
                    <ChevronDown
                      size={14}
                      className="mt-[-2px] transition-transform duration-300 ease-out group-hover:rotate-180"
                    />
                  )}

                  {/* Underline Animation */}
                  <span
                    className={`absolute -bottom-2 left-0 w-0 h-0.5 transition-[width] duration-300 ease-out group-hover:w-full ${
                      scrolled ? "bg-yellow-500" : "bg-white"
                    }`}
                  ></span>
                </Link>
          ...
          <button
            ...
            className={`md:hidden transition-colors duration-300 ease-out ${
              scrolled
                ? "text-gray-300 hover:text-yellow-500"
      ...
      <div
        className={`md:hidden absolute w-full bg-slate-900 border-b border-white/10 overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? "max-h-[80vh] opacity-100 overflow-y-auto" : "max-h-0 opacity-0"}`}
      >
                ...
                <div>
                  <div className="flex items-center justify-between w-full hover:bg-white/5 rounded-lg transition-colors duration-300">
                    ...
                    >
                      <ChevronDown
                        size={16}
                        className={`transition-transform duration-300 ease-out ${mobileSubMenuOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                  </div>

                  <div
                    className={`pl-4 space-y-1 overflow-hidden transition-all duration-500 ease-in-out ${mobileSubMenuOpen ? "max-h-96 opacity-100 mt-1" : "max-h-0 opacity-0"}`}
                  >
                    {item.dropdown.map((subItem) => (
                      <Link
                        key={subItem.name}
